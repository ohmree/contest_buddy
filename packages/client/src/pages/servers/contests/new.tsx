...
const New: Component = () => {
  const {form} = createForm({
    ...
    onSubmit: async (values: FormValues) => {
      ...
      await fetch('http://localhost:4000/api/contests', {
        method: 'POST',
    ...
    <section class="absolute top-1/2 right-1/2 transform-gpu -translate-y-1/2 translate-x-1/2 w-sm sm:w-xl md:w-2xl max-w-4xl p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
      ...
      <form use:form={form}>
        <div class="flex flex-col gap-6 mt-4">
          ...
          <div>
            <label class="text-gray-700 dark:text-gray-200" for="picturesOnly">
              Pictures only?
            </label>
            <input
              name="picturesOnly"
              type="checkbox"
              class="mx-3 p-3 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring"
            />
          </div>
